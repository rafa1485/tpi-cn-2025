clear
clc

// ============================================================================
// TRABAJO PRÁCTICO INTEGRADOR - CÁLCULO NUMÉRICO 2025
// Optimización de Climatización de Edificio Prefabricado
// ============================================================================
// Integrantes: Alan Benner; Nicolas Prosman; Mayco Chavez
//
// MÉTODOS NUMÉRICOS IMPLEMENTADOS:
// 1. Integración por Trapecio - Cálculo de energía consumida
// 2. Método de Euler - Resolución de ecuación diferencial térmica
// 3. Diferencias Finitas Centradas - Cálculo de derivadas parciales
// 4. Gradiente Descendente - Optimización de función objetivo
// ============================================================================

// DEFINICION PARA SALIDA GRAFICA
function grafico_salida(t,T,Qc,Qr,costoRefrigeracion,costoCalefaccion)
    figure()
    subplot(4,1,1)
    plot(t/3600,T)
    xlabel('Tiempo [horas]')
    ylabel('Temperatura [°C]')
    title('Evolución de la Temperatura Interior')
    xgrid()
    
    subplot(4,1,2)
    plot(t/3600,Qc,'r')
    xlabel('Tiempo [horas]')
    ylabel('Potencia [W]')
    title('Perfil de Calefacción')
    xgrid()
    
    subplot(4,1,3)
    plot(t/3600,Qr,'b')
    xlabel('Tiempo [horas]')
    ylabel('Potencia [W]')
    title('Perfil de Refrigeración')
    xgrid()
    
    subplot(4,1,4)
    xstring(0.1,0.33,"Costo Refrigeración= U$D "+string(costoRefrigeracion),0,0)
    xstring(0.1,0.66,"Costo Calefacción= U$D "+string(costoCalefaccion),0,0)
    xstring(0.1,0.1,"Costo Total= U$D "+string(costoCalefaccion+costoRefrigeracion),0,0)
endfunction

TAmbMax = 24 //"Máxima Temperatura Ambiente"
TAmbMin = 10 //"Mínima Temperatura Ambiente"
InicioSubida = 6 //"Hora en la que empieza a incrementar la temperatura"
FinSubida = 11 //"Hora en la que termina de incrementar la temperatura"
InicioBajada = 14 //"Hora en la que empieza a decrementar la temperatura"
FinBajada = 19 //"Hora en la que termina de decrementar la temperatura"


superficieEdificacion=100 // [m2]
superficiePiso=70 // [m2]

espesorEdificacion = 0.3 // [m]
coeficienteConductanciaEdificacion = 0.4 / espesorEdificacion // [W/K/m2]
conductanciaEdificacion = coeficienteConductanciaEdificacion * superficieEdificacion // [W/K]

espesorAislacionPiso = 0.05 // [m]
coeficienteConductanciaPiso = 0.02 / espesorAislacionPiso // [W/K/m2]
conductanciaPiso = superficiePiso*coeficienteConductanciaPiso // [W/K]

potenciaCalefaccionUnitaria = 10 // Potencia de calefacción por metro cuadrado [W/m2]
potenciaCalefaccion = potenciaCalefaccionUnitaria * superficiePiso // [W]
precioEnergiaCalefaccion = 0.045/1000 // [dólares/Wh] - CORREGIDO según PDF

potenciaRefrigeracionUnitaria = 5 // Potencia de refrigeración por metro cuadrado [W/m2]
potenciaRefrigeracion = potenciaRefrigeracionUnitaria * superficiePiso // [W]
precioEnergiaRefrigeracion = 0.12/1000 // [dólares/Wh]

masaUnitaria = 150 // Masa de edificio por unidad de superficie [kg/m2]
capacidadCalorificaEspecifica = 800 // Capacidad Calorífica [J/kg/K]
capacidadCalorificaUnitaria = masaUnitaria * capacidadCalorificaEspecifica // [J/K/m2]
capacidadCalorificaEdificio = capacidadCalorificaUnitaria * superficiePiso // [J/K]

h = 18 // coeficiente de transferencia de calor por convección [W/m2/K]
conductanciaConveccionEdificacion = h * superficieEdificacion;

function T_ext = T_exterior(t)
    // Temperatura exterior en función del tiempo (perfil diario)
    if t <= InicioSubida*3600 then
        T_ext = TAmbMin;
    elseif t <= FinSubida*3600 then
        T_ext = ((TAmbMax - TAmbMin)/(FinSubida - InicioSubida))*(t/3600 - InicioSubida) + TAmbMin;
    elseif t <= InicioBajada*3600 then
        T_ext = TAmbMax;
    elseif t <= FinBajada*3600 then
        T_ext = ((TAmbMin - TAmbMax)/(FinBajada - InicioBajada))*(t/3600 - InicioBajada) + TAmbMax;
    else
        T_ext = TAmbMin;
    end
endfunction

function Qp = Q_piso(T_int)
    // Calor transferido a través del piso
    Qp = conductanciaPiso * (15 - T_int);
endfunction

function Qe = Q_edif(t, T_int)
    // Calor transferido a través de paredes y techo (conducción + convección)
    T_ext = T_exterior(t) 
    Re = 1/conductanciaEdificacion; // Resistencia conducción
    Rc = 1/conductanciaConveccionEdificacion; // Resistencia convección
    conductanciaTotalEdificacion = 1/(Re + Rc);
    Qe = conductanciaTotalEdificacion * (T_ext - T_int)
endfunction

function Qc = Q_calef(t,hr_ini_cal,hr_cal)
    // Potencia de calefacción en función del tiempo
    hr_fin_cal = hr_ini_cal + hr_cal
    if (t/3600) >= hr_ini_cal && (t/3600) <= hr_fin_cal then
        Qc = potenciaCalefaccion;
    else
        Qc = 0;
    end
endfunction

function Qr = Q_refri(t,hr_ini_ref,hr_ref)
    // Potencia de refrigeración en función del tiempo (negativa = extrae calor)
    hr_fin_ref = hr_ini_ref + hr_ref
    if t/3600 >= hr_ini_ref && t/3600 <= hr_fin_ref then
        Qr = -potenciaRefrigeracion;
    else
        Qr = 0;
    end
endfunction


function Qt = Q_total(t, T_int, hr_ini_cal, hr_cal, hr_ini_ref, hr_ref)
    // Calor total: suma de todas las contribuciones térmicas
    Qp = Q_piso(T_int);
    Qe = Q_edif(t,T_int);
    Qc = Q_calef(t,hr_ini_cal,hr_cal)
    Qr = Q_refri(t,hr_ini_ref,hr_ref)
    Qt = Qp + Qe + Qc + Qr;
endfunction

function dT = f(t,T_int, hr_ini_cal, hr_cal, hr_ini_ref, hr_ref)
    // Ecuación diferencial: dT/dt = Q_total / Capacidad_Calorífica
    dT = Q_total(t,T_int, hr_ini_cal, hr_cal, hr_ini_ref, hr_ref) / capacidadCalorificaEdificio;
endfunction

// ============================================================================
// MÉTODO NUMÉRICO #1: INTEGRACIÓN POR TRAPECIO
// Descripción: Calcula la integral numérica de una función usando la regla
//              del trapecio. Se utiliza para calcular la energía consumida
//              (integral de potencia en el tiempo).
// Fórmula: integral = h/2 * [f(a) + 2*Σf(i) + f(b)]
// ============================================================================
function integral = funcion_integral(t, Q)
    n = length(t);
    h = t(2) - t(1); // Paso de tiempo constante
    
    // Fórmula del Trapecio: integral = h/2 * [f(a) + 2*sum(f(i)) + f(b)]
    integral = Q(1) + Q(n); // Primer y último término
    
    // Sumar todos los términos intermedios (multiplicados por 2)
    for i = 2:(n-1)
        integral = integral + 2*Q(i);
    end
    
    integral = integral * h / 2;
endfunction

// ============================================================================
// MÉTODO NUMÉRICO #2: MÉTODO DE EULER
// Descripción: Resuelve numéricamente la ecuación diferencial dT/dt = f(t,T)
//              para obtener la evolución de la temperatura del edificio.
// Fórmula: T(i+1) = T(i) + Δt * f(t(i), T(i))
// Implementado dentro de: funcion_costo_climatizacion y funcion_perfil_temperatura
// ============================================================================

function costoClimatizacion = funcion_costo_climatizacion(X, graficar)
    
    // DEFINICION VARIABLES DE CONTROL TEMPERATURA
    hr_ini_cal = X(1)  // Hora inicio calefacción
    hr_cal = X(2)      // Duración calefacción
    hr_ini_ref = X(3)  // Hora inicio refrigeración
    hr_ref = X(4)      // Duración refrigeración
    
    // ===== MÉTODO DE EULER: Resolución de ecuación diferencial =====
    T_ini = 19.5;      // Temperatura inicial [°C]
    T = [T_ini]        // Vector de temperaturas
    Dt = 36;           // Paso de tiempo [segundos]
    t = [0]            // Vector de tiempos
    N = (24 * 3600) / Dt;  // Número de pasos (24 horas)
    
    for i=1:N,
        t_actual = t(i);
        T_actual = T(i);
        
        // MÉTODO DE EULER: T(i+1) = T(i) + Dt * f(t(i), T(i))
        dT_dt = f(t_actual, T_actual, hr_ini_cal, hr_cal, hr_ini_ref, hr_ref);
        T_nueva = T_actual + Dt * dT_dt;
        
        T = [T, T_nueva];
        t = [t, t_actual + Dt];
    end
    
    // CALCULO DEL PERFIL DE CALOR DE CALEFACCION Y REFRIGERACION
    Qc = []
    Qr = []
    
    for i=1:length(t),
        Qc = [Qc, Q_calef(t(i), hr_ini_cal, hr_cal)];
        Qr_val = Q_refri(t(i), hr_ini_ref, hr_ref);
        Qr = [Qr, abs(Qr_val)]; // Valor absoluto para la integración
    end
    
    // ===== MÉTODO DEL TRAPECIO: Integración numérica =====
    energiaCalefaccionDiaria = funcion_integral(t, Qc) // [Joules]
    energiaRefrigeracionDiaria = funcion_integral(t, Qr) // [Joules]
    
    // Conversión a energía mensual y cálculo de costos
    energiaCalefaccionMensual_Wh = energiaCalefaccionDiaria * 30 / 3600
    costoCalefaccion = precioEnergiaCalefaccion * energiaCalefaccionMensual_Wh
    
    energiaRefrigeracionMensual_Wh = energiaRefrigeracionDiaria * 30 / 3600
    costoRefrigeracion = precioEnergiaRefrigeracion * energiaRefrigeracionMensual_Wh
    
    costoClimatizacion = costoCalefaccion + costoRefrigeracion
    
    // Solo imprimir cuando se solicite graficar
    if graficar then
        printf("\n--- RESULTADOS TÉRMICOS ---\n")
        printf("Temperatura Inicial: %.2f °C\n", T(1))
        printf("Temperatura Final: %.2f °C\n", T($))
        printf("Diferencia T_final - T_inicial: %.2f °C\n", T($) - T(1))
        printf("\n--- TIEMPOS DE OPERACIÓN ---\n")
        printf("Tiempo de Calefacción: %.2f horas\n", hr_cal)
        printf("Tiempo de Refrigeración: %.2f horas\n", hr_ref)
        printf("\n--- COSTOS MENSUALES ---\n")
        printf("Costo Calefacción: U$D %.2f\n", costoCalefaccion)
        printf("Costo Refrigeración: U$D %.2f\n", costoRefrigeracion)
        printf("Costo Total Climatización: U$D %.2f\n", costoClimatizacion)
    end

    if graficar then
        grafico_salida(t,T,Qc,Qr,costoRefrigeracion,costoCalefaccion)
    end
endfunction


function temperatura = funcion_perfil_temperatura(X)
    // Calcula solo el perfil de temperatura (sin gráficos ni costos)
    
    hr_ini_cal = X(1)
    hr_cal = X(2)
    hr_ini_ref = X(3)
    hr_ref = X(4)
    
    // ===== MÉTODO DE EULER =====
    T_ini = 19.5;
    T = [T_ini]
    Dt = 36;
    t = [0]
    N = (24 * 3600) / Dt;
    
    for i=1:N,
        t_actual = t(i);
        T_actual = T(i);
        
        dT_dt = f(t_actual, T_actual, hr_ini_cal, hr_cal, hr_ini_ref, hr_ref);
        T_nueva = T_actual + Dt * dT_dt;
        
        T = [T, T_nueva];
        t = [t, t_actual + Dt];
    end
    
    temperatura = T
endfunction

// ============================================================================
// CONDICIONES INICIALES
// ============================================================================
inicioCalefaccion = 0 
duracioCalefaccion = 10
inicioRefrigeracion = 12 
duracionRefrigeracion = 6

X = [inicioCalefaccion;
     duracioCalefaccion;
     inicioRefrigeracion;
     duracionRefrigeracion]

printf("\n========== SOLUCIÓN INICIAL ==========\n")
graficar = %T
funcion_costo_climatizacion(X, graficar);

// ============================================================================
// FUNCIÓN OBJETIVO CON PENALIZACIONES
// Descripción: Función a minimizar que incluye el costo de climatización
//              más términos de penalización para garantizar restricciones
// ============================================================================
function fcc = fobj(X)
    epsilon1 = 10   // Penalización: T_final ≈ T_inicial
    epsilon2 = 100  // Penalización: baja varianza de temperatura
    epsilon3 = 0.1  // Penalización: evitar duraciones muy cortas
    epsilon4 = 0.1
    
    // Restricciones de límites
    if X(2) <= 0 || X(4) <= 0 then
        fcc = 1e10; // Penalización grande
        return;
    end
    
    temperatura_diaria = funcion_perfil_temperatura(X)
    diferencia_cuad_inicio_fin = (temperatura_diaria($) - temperatura_diaria(1))^2
    varianza_temperatura = stdev(temperatura_diaria)^2
    fcc = funcion_costo_climatizacion(X, %F) + epsilon1 * diferencia_cuad_inicio_fin + epsilon2 * varianza_temperatura + epsilon3/X(2) + epsilon4/X(4)
endfunction

// ============================================================================
// MÉTODO NUMÉRICO #3: DERIVADAS PARCIALES POR DIFERENCIAS FINITAS CENTRADAS
// Descripción: Calcula las derivadas parciales de la función objetivo usando
//              aproximación numérica con diferencias finitas centradas.
// Fórmula: df/dx ≈ [f(x+h) - f(x-h)] / (2h)
// ============================================================================

function dfx1 = Dfx1(X)
    // Derivada parcial respecto a hr_ini_cal (hora inicio calefacción)
    h = 0.01; // Paso pequeño
    X_mas = X; X_mas(1) = X(1) + h;
    X_menos = X; X_menos(1) = X(1) - h;
    dfx1 = (fobj(X_mas) - fobj(X_menos)) / (2*h);
endfunction

function dfx2 = Dfx2(X)
    // Derivada parcial respecto a hr_cal (duración calefacción)
    h = 0.01;
    X_mas = X; X_mas(2) = X(2) + h;
    X_menos = X; X_menos(2) = X(2) - h;
    dfx2 = (fobj(X_mas) - fobj(X_menos)) / (2*h);
endfunction

function dfx3 = Dfx3(X)
    // Derivada parcial respecto a hr_ini_ref (hora inicio refrigeración)
    h = 0.01;
    X_mas = X; X_mas(3) = X(3) + h;
    X_menos = X; X_menos(3) = X(3) - h;
    dfx3 = (fobj(X_mas) - fobj(X_menos)) / (2*h);
endfunction

function dfx4 = Dfx4(X)
    // Derivada parcial respecto a hr_ref (duración refrigeración)
    h = 0.01;
    X_mas = X; X_mas(4) = X(4) + h;
    X_menos = X; X_menos(4) = X(4) - h;
    dfx4 = (fobj(X_mas) - fobj(X_menos)) / (2*h);
endfunction

function g = grad_f(X)
    // Vector gradiente: contiene las 4 derivadas parciales
    d1 = Dfx1(X)
    d2 = Dfx2(X)
    d3 = Dfx3(X)
    d4 = Dfx4(X)
    g = [d1; d2; d3; d4]
endfunction


// ============================================================================
// MÉTODO NUMÉRICO #4: GRADIENTE DESCENDENTE
// Descripción: Algoritmo de optimización que minimiza la función objetivo
//              moviéndose en la dirección opuesta al gradiente.
// Fórmula: X(k+1) = X(k) - α * ∇f(X(k))
// donde α es la tasa de aprendizaje (learning rate)
// ============================================================================
printf("\n========== OPTIMIZACIÓN POR GRADIENTE DESCENDENTE ==========\n")
alpha = 0.005      // Tasa de aprendizaje (learning rate)
max_iter = 50      // Número máximo de iteraciones
tol = 0.01         // Tolerancia para convergencia

for k = 1:max_iter
    // Calcular el gradiente en el punto actual
    grad = grad_f(X);
    
    // Actualizar X: X_nuevo = X - alpha * gradiente
    X_nuevo = X - alpha * grad;
    
    // Restricciones: mantener valores físicamente razonables
    X_nuevo(1) = max(0, min(24, X_nuevo(1))); // hr inicio calef [0-24]
    X_nuevo(2) = max(0.5, min(24, X_nuevo(2))); // duración calef [0.5-24]
    X_nuevo(3) = max(0, min(24, X_nuevo(3))); // hr inicio refrig [0-24]
    X_nuevo(4) = max(0.5, min(24, X_nuevo(4))); // duración refrig [0.5-24]
    
    // Verificar convergencia: si el cambio es pequeño, detenerse
    if norm(X_nuevo - X) < tol then
        printf("Convergencia alcanzada en iteración %d\n", k);
        break;
    end
    
    X = X_nuevo;
    printf("Iteración %d: X = [%.2f, %.2f, %.2f, %.2f]\n", k, X(1), X(2), X(3), X(4));
end

// ============================================================================
// PRESENTACIÓN DE RESULTADOS ÓPTIMOS
// ============================================================================
printf("\n========== SOLUCIÓN ÓPTIMA ==========\n");
printf("\n--- VARIABLES DE CONTROL ÓPTIMAS ---\n");
printf("X = [%.2f, %.2f, %.2f, %.2f]\n", X(1), X(2), X(3), X(4));
printf("Inicio Calefacción: %.2f horas\n", X(1));
printf("Duración Calefacción: %.2f horas\n", X(2));
printf("Inicio Refrigeración: %.2f horas\n", X(3));
printf("Duración Refrigeración: %.2f horas\n", X(4));
printf("\n--- FUNCIÓN OBJETIVO ---\n");
printf("Función Objetivo (con penalizaciones): %.2f\n", fobj(X));
printf("\n");
graficar = %T
funcion_costo_climatizacion(X, graficar);